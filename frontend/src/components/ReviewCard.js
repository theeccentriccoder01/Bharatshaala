...
const ReviewCard = ({
  ...
  const getSentimentColor = () => {
    if (review.rating >= 4) return 'text-green-600 dark:text-green-400';
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center space-x-2 mb-1">
            <h4 className="font-semibold text-gray-900 dark:text-gray-100 truncate">{review.customerName}</h4>
            {renderStars(review.rating, 'small')}
            <span className="text-gray-500 dark:text-gray-400 text-xs">{getTimeAgo()}</span>
          </div>
          <p className="text-gray-700 dark:text-gray-300 text-sm line-clamp-2">{review.comment}</p>
          {review.isVerifiedPurchase && (
            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 mt-2">
              ✓ सत्यापित खरीदारी
            </span>
          )}
        </div>
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700 ${className}`}>
      {showEditForm ? (
        ...
        <div className="space-y-4">
          ...
          {/* Title */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">शीर्षक</label>
            <input
              type="text"
          ...
          {/* Comment */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">समीक्षा</label>
            <textarea
              value={editData.comment}
              onChange={(e) => setEditData({...editData, comment: e.target.value})}
              rows={5}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 resize-none"
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center space-x-4">
              ...
              />
              <div>
                <h3 className="font-bold text-gray-900 dark:text-gray-100">{review.customerName}</h3>
                <div className="flex items-center space-x-2 mt-1">
                  {renderStars(review.rating)}
                  <span className="text-gray-500 dark:text-gray-400 text-sm">{getTimeAgo()}</span>
            ...
            <div className="flex flex-col space-y-2">
              ...
              {review.isTopReviewer && (
                <span className="bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 px-2 py-1 rounded-full text-xs font-medium">
                  👑 टॉप रिव्यूअर
                </span>
              )}
            </div>
          ...
          {review.productName && (
            <div className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
              ...
              <div>
                <h4 className="font-semibold text-gray-800 dark:text-gray-100">{review.productName}</h4>
                {review.orderDate && (
            ...
            <div className="text-gray-700 dark:text-gray-300 leading-relaxed">
              ...
              {shouldShowReadMore && (
                <button
                  ...
                  className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-200 font-medium mt-2 text-sm"
                >
              ...
              <div className="flex items-center space-x-2">
                {isEditable && user && user.id === review.userId && (
                  <>
                    <button
                      onClick={handleEdit}
                      className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 p-2 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors duration-200"
                      title="संपादित करें"
                    >
                      ✏️
                    </button>
                    <button
                      ...
                      className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-400 p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-200"
                      title="हटाएं"
                    >
                      🗑️
                    </button>
                ...
                {user && user.id !== review.userId && (
                  <button
                    ...
                    className="text-orange-600 dark:text-orange-400 hover:text-orange-800 dark:hover:text-orange-300 p-2 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors duration-200"
                    title="रिपोर्ट करें"
                  >
                    🚩
                  </button>
  ...
  const renderDefaultVariant = () => (
    <div className={`bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md border border-gray-100 dark:border-gray-700 ${className}`}>
      {/* Header */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          />
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">{review.customerName}</h3>
            <div className="flex items-center space-x-2 mt-1">
              {renderStars(review.rating, 'small')}
              <span className="text-gray-500 dark:text-gray-400 text-sm">{getTimeAgo()}</span>
            </div>
      ...
      {review.images && review.images.length > 0 && (
        <div className="flex space-x-2 mb-4">
          ...
          {review.images.length > 3 && (
            <div className="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center text-gray-500 dark:text-gray-400 text-sm">
              +{review.images.length - 3}
            </div>
          )}
        </div>
      ...
      <div className="flex items-center justify-between text-sm">
        <button
          onClick={handleHelpfulVote}
          disabled={hasVoted}
          className="flex items-center space-x-1 text-gray-600 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors duration-200"
        >
          <span>👍</span>
          <span>सहायक ({review.helpfulVotes || 0})</span>
        </button>
        ...
        {showActions && isEditable && (
          <div className="flex space-x-2">
            ...
            <button
              onClick={handleDelete}
              className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-400"
            >
              हटाएं
            </button>
          </div>
